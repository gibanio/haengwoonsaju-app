...
```
app/                 # Expo Router 페이지
├── _layout.tsx      # 네비게이션이 포함된 루트 레이아웃 (탭 네비게이션)
├── index.tsx        # 기본 리다이렉트 (luck으로 이동)
...
**WebView 기반 앱**: 메인 애플리케이션 콘텐츠는 각기 다른 웹뷰 URL에서 제공됩니다. React Native 레이어는 다음을 처리합니다:

- 네이티브 API를 통한 인앱 구매 (luck 탭에서만)
- 기기 저장소에 사진/PDF 저장 (luck 탭에서만)
- 딥링크 및 외부 URL 처리
...
앱은 4개의 주요 탭으로 구성되어 있으며, 각 탭은 서로 다른 웹뷰 URL을 로드합니다:

### 1. 행운 탭 (luck.tsx)

- **파일 위치**: `app/luck.tsx`
- **웹뷰 URL**: `Config.API_HOST` (https://dev.haengwoonsaju.com)
- **주요 기능**:
  - 인앱 구매 처리 (6개 운세 상품)
  - 이미지/PDF 저장 기능
  - 웹-네이티브 통신 (`postMessage`)
  - 스크린 캡처 및 풀 페이지 캡처
- **특징**: 가장 복잡한 기능을 포함한 메인 탭

### 2. 사주풀이 탭 (fortune.tsx)

- **파일 위치**: `app/fortune.tsx`
- **웹뷰 URL**: https://dev.haengwoonsaju.com/fortune
- **주요 기능**:
  - 기본 웹뷰 표시
  - 인스타그램 링크 외부 브라우저 처리
- **특징**: 사주 관련 콘텐츠 전용 페이지
...
- **파일 위치**: `app/mypage.tsx`
- **웹뷰 URL**: https://5hshop.com/mypage/index.php
- **주요 기능**:
  - 사용자 계정 관리
  - 마이페이지 기능
- **특징**: 쇼핑몰의 마이페이지 연동
...
- **파일 위치**: `app/index.tsx`
- **기능**: 앱 시작 시 `luck` 탭으로 자동 리다이렉트
- **코드**: `<Redirect href="/luck" />`

### 탭 네비게이션 구성

앱은 하단 탭바로 구성되며, 각 탭의 아이콘과 제목은 다음과 같습니다:

1. **행운** (luck) - `leaf` 아이콘
2. **사주풀이** (fortune) - `star` 아이콘
3. **오행샵** (shop) - `storefront` 아이콘
4. **마이** (mypage) - `person` 아이콘

### 공통 기능

모든 웹뷰 탭에서 공통으로 제공하는 기능:

- iOS WebView 스케일링 문제 방지 JavaScript 주입
- 인스타그램 링크 외부 브라우저 처리
- 뷰포트 설정 및 제스처 처리
- 탭바 네비게이션 (`_layout.tsx`에서 관리)
...
- `API_HOST`: 메인 웹 애플리케이션 URL (https://dev.haengwoonsaju.com)
