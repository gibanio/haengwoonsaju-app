...
"행운사주"는 Expo React Native로 구축된 한국 운세 모바일 애플리케이션입니다. 이 앱은 PG 결제 시스템과 이미지/PDF 저장 기능이 포함된 다양한 운세 서비스를 제공합니다.
...
- **주요 기능**:
  ...
  - 인스타그램 링크 외부 브라우저 처리
  - 탭바 동적 표시/숨김
- **특징**: PG 결제가 통합된 사주 관련 콘텐츠 전용 페이지
...
- **iOS**: 사진 라이브러리 권한, 번들 식별자 `com.haengwoonsaju`, PG 결제용 URL 스킴
...
- PG 결제 플로우: WebView 내 처리 → 콜백 URL 감지 → 결과 전달
...
PG 결제는 WebView 내에서 처리되며, 결제 완료 시 콜백 URL을 통해 결과를 받아 WebView로 전달합니다. 카드사 앱 호출을 위한 URL 스킴 처리가 필요합니다.
